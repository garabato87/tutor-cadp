...
export function DashboardView({
  ...
  return (
    <div className="animate-fade-up space-y-6 motion-reduce:animate-none">
      {/* Hero */}
      <section className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          {countdown && (
            <div className="flex shrink-0 items-center gap-3 rounded-xl border border-border bg-muted/60 px-4 py-3">
              <Clock className="h-5 w-5 text-teal" aria-hidden />
              <div>
        ...
        <div className="relative mt-6 rounded-xl border border-border bg-background/40 p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="flex items-center gap-2 text-sm font-medium text-foreground">
              <TrendingUp className="h-4 w-4 text-primary" aria-hidden /> Progreso total
            </span>
            <button
              onClick={onOpenProgreso}
              aria-label="Ver el detalle del progreso total"
              className="font-mono text-xs text-muted-foreground transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >
              ver detalle →
            </button>
          </div>
        ...
        <div className="relative mt-5 flex flex-wrap items-center gap-3">
          ...
          <span className="inline-flex items-center gap-2 font-mono text-xs text-muted-foreground">
            <proximoMeta.Icon className="h-3.5 w-3.5" style={{ color: `hsl(${proximoMeta.accent})` }} aria-hidden />
            {proximo.titulo}
          </span>
        </div>
      </section>
